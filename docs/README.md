#  🎨 MUSINSA AI 기반 패션 스타일 매칭 및 토탈 아웃핏 추천 서비스
## 📚 Documentation
[👉 **Team 데스노트 Notion 워크스페이스 바로가기**](https://www.notion.so/2a8d4d635aa8801788b6d4b04a07a456?source=copy_link)
> 프로젝트의 기획안과 개발 일지는 위 링크에서 확인하실 수 있습니다.

## 📌 프로젝트 개요

MUSINSA_PERSONA은 사용자의 패션 선호도와 라이프스타일을 분석하여 개인화된 패션 스타일(페르소나) 기반의 상품 추천을 제공하는 AI 웹 애플리케이션입니다.<br>
사용자는 직관적인 인터페이스를 통해 자신의 스타일을 진단받고, AI가 추천한 상품들을 조합하여 가상 코디(아웃핏)를 완성한 후 구매할 수 있습니다.
MUSINSA_PERSONA은 MUSINSA에 등록된 상품을 활용해 제작한 서비스입니다.

## ✨ 핵심 기능

📝 **스타일 진단**: 8개 질문을 통해 사용자의 패션 페르소나 유형을 16가지로 분류
...
## 🔄 핵심 로직 상세 (Core Logic)
### 1. 스타일 진단 알고리즘
- Step 1 (성향 분류): 초기 4개 문항의 응답에 따라 4가지 성향(A/B/C/D) 중 최고점을 차지한 성향을 도출합니다
  - A: 클래식/고급 (올드머니, 오피스룩)
  - B: 기능성/내구성 (워크웨어, 고프코어)
  - C: 개성/트렌드 (Y2K, 스트릿)
  - D: 편안함/활동성 (블록코어, 시티보이)

- Step 2 (페르소나 결정): 도출된 성향 내에서 심화 질문 4가지를 통해 최종 16개 페르소나 중 하나를 결정합니다.
...
#### 3. 마스터 데이터 생성 (최초 1회)
*** 후술한 스키마와 그에 대응되는 .npz파일이 존재해야합니다. ***
...
| **brand_vecs** | (N, 768) | 브랜드 설명 텍스트 임베딩 |
| **img_vecs** | (N, 512) | 상품 이미지 임베딩 (CLIP) |
| **cat_vecs** | (N, 50) | 카테고리 임베딩 |
